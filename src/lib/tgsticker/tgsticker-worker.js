var rlottieRuntimeReady = false;

var Module = self.Module || {};
self.Module = Module;
Module.onRuntimeInitialized = function() {
  rlottieRuntimeReady = true;
  if (typeof RLottieWorker !== 'undefined') {
    RLottieWorker.init();
  }
};

var workerParams = new URL(self.location.href).searchParams;
Module.locateFile = function(path) {
  return path === 'rlottie-wasm.wasm' ? (workerParams.get('wasm') || path) : path;
};
importScripts(workerParams.get('runtime') || 'rlottie-wasm.js');
importScripts(workerParams.get('pako') || 'pako-inflate.min.js');
